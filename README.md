...
- **SU-consuming kinetic block + goggle overlay** — `content/kinetics/CreatePOSTMORTEMKineticBlock` +
  `CreatePOSTMORTEMKineticBlockEntity`. An encased-shaft-style block that joins the kinetic
  ...
  Its model is an inset casing and `CreatePOSTMORTEMShaftRenderer` spins a Create shaft that pokes
...
- **Kinetic generator + goggle overlay** — `content/kinetics/CreatePOSTMORTEMGeneratorBlock` +
  `CreatePOSTMORTEMGeneratorBlockEntity`. The counterpart source: it *adds* stress capacity
  ...
  network and nothing turns. It shares `CreatePOSTMORTEMShaftRenderer` and adds custom lines to the
  Engineer's Goggles tooltip (`IHaveGoggleInformation#addToGoggleTooltip`).
- **Display Link source** — `content/display/CreatePOSTMORTEMDisplaySource` + `AllDisplaySources`.
...
- **Sequenced assembly recipe** — `datagen/CreatePOSTMORTEMSequencedAssemblyGen`. An item is
  processed through a Deployer then a Mechanical Press, looping twice, using a
  transitional "incomplete" item.
- **Ponder plugin** — `content/ponder/CreatePOSTMORTEMPonderPlugin` +
  `CreatePOSTMORTEMPonderScenes`. Wires up Create's in-game animated manual for the addon and
  is registered client-side in `CreatePOSTMORTEM#onClientSetup`. Ships a worked scene driven
  by the `assets/postmortem/ponder/postmortem_ponder.nbt` schematic (two Mechanical Arms and
  ...
  (`CreatePOSTMORTEM#registerPonderLang`), so it appears after `./gradlew runData`.

- **Create-style item tooltip** — the `postmortem_item`'s tooltip is authored as lang keys in
  `assets/postmortem/lang/default/tooltips.json` (`.tooltip.summary` +
  `.tooltip.condition1`/`.behaviour1`). The `ItemDescription` modifier set up on the
  Registrate in `CreatePOSTMORTEM` reads them and adds the "Hold Shift" prompt and highlight
...
- **Lang: keys in code, copy in JSON** — like Create, English strings live in hand-authored
  partials under `assets/postmortem/lang/default/` (`interface.json`, `tooltips.json`), while
  `Lang` holds only translation keys. `datagen/CreatePOSTMORTEMLangMerger` merges the partials into the
  ...
  `CreatePOSTMORTEMLangMerger`) — never by putting English in Java.
- **Additional languages** — `assets/postmortem/lang/de_de.json` is a worked German
...
It clones this template and rebrands every `Example` / `postmortem` reference to your
...
Run the bootstrap task once — it rewrites every `Example` / `postmortem` reference,
...
2. Rename `src/main/java/com/example/postmortem/` to match your `mod_group_id`
3. Update `CreatePOSTMORTEM.java` — change `ID` to your `mod_id`
4. Rename `src/main/resources/postmortem.mixins.json` to `{mod_id}.mixins.json` and
...
1. Go into your CreatePOSTMORTEM.java class to get a quick overview over what is defined and where exactly.
